...
class RefreshMailboxCommand extends Command
{
                    ...
                    if ($imapConfiguration instanceof IMAP\Transport\AppTransportConfigurationInterface) {
                        ...
                        if (empty($microsoftApp)) {
                            ...
                            if (empty($microsoftAccount)) {
                                $output->writeln(
                                    "  <comment>No microsoft account was found with email '" . $imapConfiguration->getUsername() . "' for configured client id '" . $imapConfiguration->getClient(
                                    ) . "'.</comment>"
                                );
    ...
    public function refreshMailbox($server_host, $server_username, $server_password, \DateTime $timestamp, OutputInterface $output)
    {
        ...
        if (true === is_array($emailCollection)) {
            ...
            foreach ($emailCollection as $id => $messageNumber) {
                $output->writeln(sprintf('    - <comment>Processing email</comment> <info>%d</info> <comment>of</comment> <info>%d</info>:', $counter, $emailCount));
                ...
                [$response, $responseCode, $responseErrorMessage] = $this->parseInboundEmail($message, $output);

                if (200 === $responseCode) {
                    $output->writeln("\n      <bg=green;fg=bright-white;options=bold>200</> " . $response['message'] . "\n");
                } else {
                    if (false === empty($responseErrorMessage)) {
    ...
    public function refreshOutlookMailbox(MicrosoftApp $microsoftApp, MicrosoftAccount $microsoftAccount, \DateTime $timestamp, OutputInterface $output)
    {
        ...
        if (!empty($response['value'])) {
            ...
            foreach ($response['value'] as $message) {
                ...
                [$response, $responseCode, $responseErrorMessage] = $this->parseOutlookInboundEmail($detailedMessage, $output);
    ...
    public function parseInboundEmail(string $message, OutputInterface $output): array
    {
    ...
    private function writeImapErrors(OutputInterface $output)
    {
        $errorMessages = imap_errors();

        if (0 === count($errorMessages)) {
            $output->writeln("  - <fg=red>IMAP failed without errors</>");

            return;
        }

        $output->writeln("  - <fg=red>IMAP Errors:</>");
        foreach ($errorMessages as $id => $errorMessage) {
            $output->writeln(sprintf('  <comment>%s: %s</comment>', $id, $errorMessage));
        }
    }
}
