...
class MailboxService
{
    ...
    public function __construct(ContainerInterface $container, EntityManagerInterface $entityManager, TicketConversationRepository $ticketConversationRepository)
    {
        ...
        foreach ($mailboxes as $id) {
            ...
            if (!empty($params['smtp_server'])) {
                ...
                if ($smtpConfiguration instanceof SMTP\Transport\AppTransportConfigurationInterface) {
                    ...
                    if ($smtpConfiguration instanceof SMTP\Transport\ResolvedTransportConfigurationInterface) {
                        $smtpConfiguration
                            ->setUsername($params['smtp_server']['username'])
    ...
    private function getRegisteredMailboxes(): array
    {
    ...
    private function searchExistingTickets(array $criterias = []): ?Ticket
    {
        if (true === empty($criterias)) {
            return null;
        }
        ...
        foreach ($criterias as $criteria => $criteriaValue) {
            if (true === empty($criteriaValue)) {
                continue;
            }

            switch ($criteria) {
                case 'messageId':
                    $ticket = $ticketRepository->findOneBy(['referenceIds' => $criteriaValue]);
                ...
                case 'outlookConversationId':
                    $ticketConversation = $this->ticketConversationRepository->findOneByConversationId($criteriaValue);
                    if (null !== $ticketConversation?->getTicket()) {
                        return $ticketConversation->getTicket();
                    }

                    $ticket = $ticketRepository->findOneBy(['outlookConversationId' => $criteriaValue]);
    ...
    public function processMail($rawEmail)
    {
        ...
        if (empty($addresses['from'])) {
            ...
            if (null !== $this->getMailboxByEmail($addresses['from'])) {
                return [
        ...
        if (empty($ticket)) {
            ...
            if (false === $ticket->getIsTrashed() && strtolower($ticket->getStatus()->getCode()) != 'spam' && !empty($mailData['inReplyTo'])) {
                ...
                if ($ticket->getCustomer() && $ticket->getCustomer()->getEmail() == $mailData['from']) {
                    ...
                    if ($this->entityManager->getRepository(Ticket::class)->isTicketCollaborator($ticket, $mailData['from'])) {
                ...
                if ($thread->getThreadType() == 'reply') {
                    if ($thread->getCreatedBy() == 'customer') {
                        ...
                        if ($thread->getCreatedBy() == 'collaborator') {
    ...
    public function processOutlookMail(array $outlookEmail): array
    {
        $mailData = [];
        $senderName = null;
        ...
        if (!empty($outlookEmail['from']['emailAddress']['address'])) {
            ...
            if (!empty($outlookEmail['sender']['emailAddress']['address'])) {
        ...
        // Check for self-referencing. Skip email processing if a mailbox is configured by the sender's address.
        if (null !== $this->getMailboxByEmail($senderAddress)) {
            /* Create conversation if not already exists */
            if (true === array_key_exists('internetMessageHeaders', $outlookEmail)) {
                $referenceIdIndex = array_search('x-References', array_column($outlookEmail['internetMessageHeaders'], 'name'));
                if (false !== $referenceIdIndex && false === empty($outlookEmail['internetMessageHeaders'][$referenceIdIndex]['value'])) {
                    $referenceId = $outlookEmail['internetMessageHeaders'][$referenceIdIndex]['value'];

                    $existingConversation = $this->ticketConversationRepository->findOneByConversationId($outlookEmail['conversationId']);
                    if (null === $existingConversation) {
                        $ticket = $this->ticketRepository->findOneBy(['referenceIds' => $referenceId]);
                        if (null !== $ticket) {
                            $ticketConversation = new TicketConversation();
                            $ticketConversation
                                ->setTicket($ticket)
                                ->setConversationId($outlookEmail['conversationId']);

                            $this->entityManager->persist($ticketConversation);
                            $this->entityManager->flush();
                        }
                    }
                }
        ...
        return [
            ...
            'content' => [
                ...
                'thread'    => !empty($thread) ? $thread->getId() : null,
                'ticket'    => !empty($ticket) ? $ticket->getId() : null,
                'messageId' => (false === empty($mailData['messageId'])) ? $mailData['messageId'] : null,
            ],
        ];
    }
}
